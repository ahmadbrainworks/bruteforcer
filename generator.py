...
print (" \n ")
print (Fore.YELLOW + "                 WordList generator For BruteFORCER")
...
def write_words_from_list(all_words_list):
    ...
    if(file_existed_checked==0):
        filename="passwords-{}.txt".format(datetime.now().strftime('%H-%M'))
...
def file_existed(filename_for_check,counter=0):
    ...
    if(os.path.isfile(filename_for_check)):
        ...
        new_filename="passwords-{}({}).txt".format(datetime.now().strftime('%H-%M'),new_counter)
